...
class Program
{
    static void Main(string[] args)
    {
        ...
        var comboBox = new ComboBox(new string[] { "CPU-powered Phi-3-mini", "DirectML-powered Phi-3-mini" });
        ...
        comboBox.Changed += (sender, e) =>
        {
            ...
            try
            {
                model = new Model(modelPath);
                tokenizer = new Tokenizer(model);
                textView.Buffer.Text += $"Model switched to: {(comboBox.Active == 0 ? "CPU-powered Phi-3-mini" : "DirectML-powered Phi-3-mini")}\n";
            }
            catch (Exception ex)
            {
                textView.Buffer.Text += $"Error loading model: {ex.Message}\n";
